...
void GameActionManager::onMainCardClicked(CardWidget* cardWidget)
{
    switch (cardWidget->cardData().type) {
        ...
        //Choose two cards
         case CARD_ARSON:
             //selectCardsAndPlayer(cardWidget, 1);
                 selectCards(cardWidget, 2);
                 break;
        default:
...
{
     if ((mp_activeCard->pocketType() == POCKET_HAND) && (mp_activeCard->cardData().type == CARD_ARSON)){
        //Needs played to more players
            QList<int> playerIds = QList<int>();
            foreach (CardWidget* card2, m_cardSelection){ 
                playerIds.append(card2->ownerId());
            }
            mp_game->serverConnection()->playCardWithPlayers(mp_activeCard->cardData().id, playerIds); 
    }
    else if (m_cardSelection.size() == 1) {
