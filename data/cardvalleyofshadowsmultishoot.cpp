...
void CardValleyOfShadowMultiShoot::respondPass()
{
    ...
    if (type() == CARD_TORNADO){
        ...
        /*qDebug() << "CardValleyOfShadowMultiShoot::respondPass 1";
        if (gameCycle()->gamePlayState() == GAMEPLAYSTATE_RESPONSE){
            qDebug() << "CardValleyOfShadowMultiShoot::respondPass 2";
            gameCycle()->unsetResponseMode();
            qDebug() << "CardValleyOfShadowMultiShoot::respondPass 3";
        }
        qDebug() << "CardValleyOfShadowMultiShoot::respondPass 4";
        mp_requestedPlayer->modifyLifePoints(-1, mp_shootingPlayer);
        qDebug() << "CardValleyOfShadowMultiShoot::respondPass 5";
        if (mp_requestedPlayer == mp_shootingPlayer) return;
        qDebug() << "CardValleyOfShadowMultiShoot::respondPass 6";
        gameTable()->playerPass(mp_requestedPlayer);
        qDebug() << "CardValleyOfShadowMultiShoot::respondPass 7";
        //requestNext();
        qDebug() << "CardValleyOfShadowMultiShoot::respondPass 8";
        //return;*/
        if (gameCycle()->gamePlayState() == GAMEPLAYSTATE_RESPONSE){
            gameCycle()->unsetResponseMode();
        }
