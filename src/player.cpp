...
void Player::setRoleAndCharacter(CharacterBase* character, const PlayerRole& role)
{
    ...
    //If you don't have any ability as sheriff, have you health
    if ((role == ROLE_SHERIFF) && (!character->ifCanBeTheSeriff())){
        m_maxLifePoints++;
      //throw std::invalid_argument("This character can't be the sheriff!");
    }
    m_lifePoints = m_maxLifePoints;
}
