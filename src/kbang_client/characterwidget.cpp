...
void CharacterWidget::init(CardWidgetFactory* cardWidgetFactory)
{
    ...
    QSize widgetSize(CardWidget::qSize(CardWidget::SIZE_SMALL).width(),
                         (int)(CardWidget::qSize(CardWidget::SIZE_SMALL).height() + mp_backCard->lifeLevel(10)));
...
{
    if (lifePoints < 0 || lifePoints > 10) return;
...
void CharacterWidget::lifePointsChanged()
{
    ...
    m_targetY = mp_backCard->y() + mp_backCard->lifeLevel(m_lifePoints);
    if ((m_lifePoints < 6) && (m_lifePoints > 0)) {
        mp_characterCard->hide();
        mp_backCard = mp_cardWidgetFactory->createCharacterCard(this ,CHARACTER_UNKNOWN5);
        m_targetY = m_targetY + mp_backCard->lifeLevel(m_lifePoints);
    }
    else if (m_lifePoints >= 6){
        mp_characterCard->hide();
        mp_backCard = mp_cardWidgetFactory->createCharacterCard(this);
    }
    mp_backCard->validate();
    mp_characterCard = mp_cardWidgetFactory->createCharacterCard(this);
    mp_characterCard->setCharacterType(m_character);
    mp_characterCard->validate();
    mp_backCard->setVisible(true);
    mp_characterCard->setVisible(true);
    m_time.start(); 
