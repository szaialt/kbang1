...
void PlayerWidget::setFromPublicData(const PublicPlayerData& publicPlayerData)
{
    ...
    m_countOfLivingPlayers = publicPlayerData.countOfLivingPlayers;
...
{
    table()->clear();
    qDebug() << "PlayerWidget::setTable(QList<CardData> tableCards)" <<  tableCards.size();
    foreach (const CardData& cardData, tableCards) {
        CardWidget* card = mp_game->cardWidgetFactory()->createPlayingCard(this);
        card->setCardData(cardData);
        qDebug() << "CLIENT: " << pocketTypeToString(cardData.pocket);
        card->validate();
        table()->push(card);
    }
}
