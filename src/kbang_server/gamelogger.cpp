...
void GameLogger::onPlayerDied(PublicPlayerView& p)
{
    QString msg = QString("onPlayerDied(player=%1)").
                  arg(p.id());
    m_logFile << msg.toStdString() << endl;
}
...
void GameLogger::onCancelCard(PocketType, const PlayingCard* c)
{
    QString msg = QString("onCancelCard(card=%1)").
                  arg(cardToString(c));
    m_logFile << msg.toStdString() << endl;
}
