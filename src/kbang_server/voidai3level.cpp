                ...
                try {
                    switch(card->type()) {
                        ...
                        { 
                            ...
                            if ((mp_playerCtrl->role() == ROLE_SHERIFF) && sheriff->game()->alivePlayersCount() > 2){
                                 qDebug() << "ROLE_SHERIFF";
                                 int deputyNumber = 1;
                                 int calculator = 0;
                            ...
                            {
                                if (sheriff->game()->alivePlayersCount() > 2){
                                  if (sheriff->publicView().lifePoints() > 2){
                                    mp_playerCtrl->playCard(card);
                                  }
                                  else {
                                    continue;
...
void VoidAI3Level::stealCard(PlayingCard* card, PublicPlayerView* targetPlayer){
    qDebug() << "stealCard: Card type: " << playingCardTypeToString(card->type());
    bool goodThing = true;
    if (targetPlayer->role() == ROLE_SHERIFF){
        if (mp_playerCtrl->role() == ROLE_DEPUTY) goodThing = false;
        if ((mp_playerCtrl->role() == ROLE_RENEGADE) && (!inEndGame()))
            goodThing = false;
    }
    else if (targetPlayer->role() == mp_playerCtrl->role()){
        goodThing = false;
    }
    else {
        
        QList<PublicPlayerView*> adversaries = targetPlayer->mp_player->adversaries();
        bool isOutlaw = false; 
        PublicPlayerView* q;
        foreach(q, adversaries){
            if (q->role() == ROLE_SHERIFF){
                isOutlaw = true;
                qDebug() << "Suspected " << playerRoleToString(targetPlayer->role()) << "to be an outlaw.";
                }
            }
            if (mp_playerCtrl->role() == ROLE_OUTLAW){
                if (isOutlaw) goodThing = false;
            }
            if ((mp_playerCtrl->role() == ROLE_DEPUTY) || (mp_playerCtrl->role() == ROLE_SHERIFF)){
                if (!isOutlaw) goodThing = false;
            }
    }
    bool goodCard = true;
    PlayingCard* targetCard;
    if (targetPlayer->player()->table().size() > 0){
        targetCard = targetPlayer->player()->getRandomCardFromTable();
        if (targetCard == 0) return;
        if (targetCard->type() == CARD_JAIL) goodCard = false;
        if (targetCard->type() == CARD_GUITAR) goodCard = false;
        if (targetCard->type() == CARD_JARATE) goodCard = false;
    }
    else if (targetPlayer->player()->handSize() > 0){
        if (goodCard){
            if (targetPlayer->id() == mp_playerCtrl->publicPlayerView().id()) return;
            targetCard = targetPlayer->player()->getRandomCardFromHand();
        }
        if (targetCard == 0) return;
    }
    else {
    ...
    if (targetCard->pocket() == POCKET_GRAVEYARD) return;
    if (card->pocket() == POCKET_GRAVEYARD) return;
    if (goodThing == goodCard){
