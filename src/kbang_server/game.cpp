...
int Game::getDistance(Player *fromPlayer, Player *toPlayer) const
{
    ...
    if (fromPlayer->characterType() == CHARACTER_PRESTON_PRESBYOPE){
        if (baseDistance < 2){
            if (numberOfLivingPlayers > 3){
                return infiniteDistance;
            }
            else {
                baseDistance++;
...
void Game::buryPlayer(Player* player, Player* causedBy)
{
    ...
    QList<PlayingCard*> table = player->table();
        ...
        } else if ((player->role() == ROLE_OUTLAW && causedBy != 0 && causedBy != 0 && causedBy != player) || (causedBy != 0 && !(causedBy->isCharmed()) && ((causedBy->characterType() == CHARACTER_LOCO) || (causedBy->characterType() == CHARACTER_SCALPING_BULL)))) {
