...
bool GameServer::listen()
{
    ...
    {
        qCritical("ERROR: Unable to listen on %s:%d", mp_tcpServer->hostAddressString().toStdString().c_str(),
                  mp_tcpServer->port());
        return 0;
    }
    else
    {
        qWarning("Listening on %s:%d", mp_tcpServer->hostAddressString().toStdString().c_str(), mp_tcpServer->port());
    }
    return 1;
}
