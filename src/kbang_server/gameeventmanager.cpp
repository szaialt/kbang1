...
void GameEventManager::onPlayerDied(Player* p, Player* causedBy)
{
    ...
    foreach(Handler* h, m_handlers) {
        if (causedBy) {
            ...
            h->handler->onPlayerDied(p->publicView(), &causedBy->publicView());
           
        }
        else {
           
            h->handler->onPlayerDied(p->publicView());
...
void GameEventManager::onDrawIntoSelection(QList<const PlayingCard*> cards)
{
    foreach(Handler* h, m_handlers) {
            h->handler->onDrawIntoSelection(cards);
    }
...
void GameEventManager::onCancelCard(Player* targetPlayer, PocketType pocketFrom, PlayingCard* card, Player* causedBy)
{
    foreach(Handler* h, m_handlers) {
        //Segfault!
        //Relocate if-then from client 
        if (targetPlayer && causedBy) {
            h->handler->onCancelCard(pocketFrom, card, &targetPlayer->publicView(), &causedBy->publicView());
        }
        else {
        ...
        //onPlayerUpdated(h->player);
//         h->handler->onCancelCard(pocketFrom, card,
//                                    targetPlayer ? &targetPlayer_loc->publicView() : 0,
//                                     causedBy ? &causedBy_loc->publicView() : 0);
    }
}
