...
{
    QByteArray bytes = QByteArray::fromBase64(node->getFirstChild()->text().toStdString().c_str());
...
void GameInfoData::read(XmlNode* node)
{
    ...
    /*characterNumber   = node->attribute("characterNumber").toInt();
    characterChoice     = node->attribute("characterChoice") == "true";  */  
    
     state               = stringToGameState(node->attribute("state"));
     lifePointsMaxFlag = false;
     lifePointsMaxFlag   = (node->attribute("lifePointsMaxFlag")) == "true";
     dodgeCityFlag = false;
     dodgeCityFlag      = (node->attribute("dodgeCityFlag")) == "true";
     teamFortressFlag = false;
     teamFortressFlag      = (node->attribute("teamFortressFlag")) == "true";
     ourFlag = false;
     ourFlag            = (node->attribute("ourFlag")) == "true";
     stackingDynamiteFlag = false;
...
//      writer->writeAttribute("characterNumber",       QString::number(characterNumber));
//      writer->writeAttribute("characterChoice",       characterChoice ? "true" : "false");
    
     writer->writeAttribute("lifePointsMaxFlag",        lifePointsMaxFlag ? "true" : "false");
      writer->writeAttribute("dodgeCityFlag",        dodgeCityFlag ? "true" : "false");
      writer->writeAttribute("teamFortressFlag",        teamFortressFlag ? "true" : "false");
...
void CardData::read(XmlNode* node)
{
    ...
    //isAct       = false;
    isAct       = node->attribute("isAct") == "true";
    pocket      = stringToPocketType(node->attribute("pocket"));
}
...
void CardData::write(QXmlStreamWriter* writer) const
{
    ...
    writer->writeAttribute("isAct",     isAct ? "true" : "false");
    writer->writeAttribute("pocket",    pocketTypeToString(pocket));
    writer->writeEndElement();
}
...
void PublicPlayerData::read(XmlNode* node)
{
    ...
    foreach(XmlNode* child, node->getChildren()) {
        ...
        else if (child->name() == "characters") {
            characters.clear();
            foreach(XmlNode* character2, child->getChildren()) {
                //stringToCharacterType(character2->attribute("character"));
                 //= stringToCharacterType(node->attribute("character"));
                CharacterType characterType;
                characterType = stringToCharacterType(character2->attribute("character"));
                characters.push_back(characterType);
            }
        } 
...
void PublicPlayerData::write(QXmlStreamWriter* writer) const
{
    ...
    foreach(const CharacterType& character, characters)
        writer->writeAttribute("character",         characterTypeToString(character));
    writer->writeEndElement();
    writeAvatar(writer, avatar);
    writer->writeEndElement();
...
void ActionPlayCardData::read(XmlNode* node)
{
    ...
    if (typeString == "PLAYCARD_CARD_AND_PLAYER") {
        type = PLAYCARD_CARD_AND_PLAYER;
        targetCardId = node->attribute("target-card-id").toInt();
        targetPlayerId = node->attribute("target-player-id").toInt();
    }    
    else if (typeString == "PLAYCARD_PLAYER") {
        type = PLAYCARD_PLAYER;
        targetPlayerId = node->attribute("target-player-id").toInt();
    } 
    else if (typeString == "PLAYCARD_CARD") {
        type = PLAYCARD_CARD;
        targetCardId = node->attribute("target-card-id").toInt();
    } 
    else if (typeString == "PLAYCARD_HAND") {
        type = PLAYCARD_HAND;
...
void ActionPlayCardData::write(QXmlStreamWriter* writer) const
{
    ...
    case PLAYCARD_SIMPLE:
        writer->writeAttribute("type", "PLAYCARD_SIMPLE");
        break;
    case PLAYCARD_PLAYER:
        writer->writeAttribute("type", "PLAYCARD_PLAYER");
        writer->writeAttribute("target-player-id", QString::number(targetPlayerId));
        break;
    case PLAYCARD_CARD:
        writer->writeAttribute("type", "PLAYCARD_CARD");
        writer->writeAttribute("target-card-id", QString::number(targetCardId));
        break;
    case PLAYCARD_HAND:
        writer->writeAttribute("type", "PLAYCARD_HAND");
        writer->writeAttribute("target-hand-id", QString::number(targetHandId));
        break;
    case PLAYCARD_CARD_AND_PLAYER:
        writer->writeAttribute("type", "PLAYCARD_CARD_AND_PLAYER");
        writer->writeAttribute("target-card-id", QString::number(targetCardId));
        writer->writeAttribute("target-player-id", QString::number(targetPlayerId));
        break;
    case PLAYCARD_CARDS:
        writer->writeAttribute("type", "PLAYCARD_CARDS");
        ...
        break;
    case PLAYCARD_PLAYERS:
        writer->writeAttribute("type", "PLAYCARD_PLAYERS");
        foreach(int playerId, targetPlayersId) {
            writer->writeStartElement("player");
            writer->writeAttribute("id", QString::number(playerId));
            writer->writeEndElement();
        }
        break;
    }
...
void ActionUseAbilityData::read(XmlNode* node)
{
    ...
    if (typeString == "TypeSimple"){
        type = ActionUseAbilityData::TypeSimple;
    }
    else if (typeString == "TypePlayer"){
        type = ActionUseAbilityData::TypePlayer;
        targetPlayerId = node->attribute("target-player-id").toInt();
    }
    else if (typeString == "TypeCards"){
        type = ActionUseAbilityData::TypeCards;
        foreach (XmlNode* cardNode, node->getChildren()) {
            targetCardsId.append(cardNode->attribute("id").toInt());
        }
    }
    else if (typeString == "TypeCardsAndPLayer"){
        type = ActionUseAbilityData::TypeCardsAndPLayer;
        targetPlayerId = node->attribute("target-player-id").toInt();
        foreach (XmlNode* cardNode, node->getChildren()) {
            targetCardsId.append(cardNode->attribute("id").toInt());
        }
...
void ActionUseAbilityData::write(QXmlStreamWriter* writer) const
{
    ...
    switch(type) {
    case TypeSimple:
        writer->writeAttribute("type", "TypeSimple");
        break;
    case TypePlayer:
